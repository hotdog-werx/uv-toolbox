...
def test_lock_writes_lockfile(mocker: MockerFixture, tmp_path: Path) -> None:
    venv_root = tmp_path / '.uv-toolbox'
    config_path = _write_config(tmp_path, venv_path=venv_root, envs=[('env1', 'ruff')])
    lock_data = UvToolboxLock(environments={'env1': EnvironmentLock(requirements='ruff==0.14.14\n')})
...
def test_lock_handles_uv_toolbox_error(mocker: MockerFixture, tmp_path: Path) -> None:
    venv_root = tmp_path / '.uv-toolbox'
    config_path = _write_config(tmp_path, venv_path=venv_root, envs=[('env1', 'ruff')])

    mocker.patch('uv_toolbox.cli.generate_lock', side_effect=UvToolboxError('compile failed'))
