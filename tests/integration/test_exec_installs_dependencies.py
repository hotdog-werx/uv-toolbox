...
def test_integration_exec_installs_project_dependencies(
    ...
    """Test that uvtb exec properly isolates the venv.

    This test verifies that when running `uvtb exec`, only explicitly
    listed dependencies are installed, and project dev dependencies are
    NOT auto-installed by uv's project detection.
    """
    ...
    # Verify no installation activity - uvtb exec should use the existing venv
    # without triggering any package installations
    assert 'nstalled' not in exec_result.stderr, (
        f'Expected to not see installation activity in stderr, but got:\n{exec_result.stderr}'
    )
    ...
    # CRITICAL VERIFICATION: Check that pytest (from dev dependencies) was NOT installed!
    # This proves that uvtb exec properly isolates the venv and doesn't auto-install
    # optional dependencies (dev group) from the project's pyproject.toml.
    pytest_import_check = subprocess.run(
        ['uvtb', 'exec', '--', 'python', '-c', 'import pytest'],
        check=False,
        capture_output=True,
        text=True,
    )

    # pytest should NOT be importable - it's only in dev dependencies, not in requirements
    assert pytest_import_check.returncode != 0, (
        f'pytest should NOT be installed in the venv!\n'
        f'stdout: {pytest_import_check.stdout}\nstderr: {pytest_import_check.stderr}'
    )
    assert 'ModuleNotFoundError' in pytest_import_check.stderr or 'No module named' in pytest_import_check.stderr, (
        f'Expected ModuleNotFoundError for pytest, got:\n{pytest_import_check.stderr}'
    )
    ...
    # And it's NOT available in the venv! This means uvtb exec properly isolated the environment.
